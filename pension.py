...
df_tot = pd.DataFrame([], columns=['step', 'start_date', 'tickers', 'knives', 'slip', 'multi', 'buy_and_hold_gain',
        ...
        for TICKERS in [1]:
            for KNIVES in [0]:
                for SLIP in [-1, 0]:
                    for MULTI in [0]:
                        print(
                            f'STEP {STEP} start_date {start_date} TICKERS {TICKERS} KNIVES {KNIVES} SLIP {SLIP} MULTI {MULTI}')
                        ...
                        start_index = 240
                        end_index = len(datas[tickers[0]]) - STEP - 5
                        for index in range(start_index, end_index, STEP):
                            ...
                            for ticker in tickers:
                                ...
                                # score += price_now/price_6m * MULTI
                                # score -= price_now/price_12m * 1
                            ...
                            pass
                        df.to_csv(f'results/dipper.csv')
                        sheet_name = f'{start_date}s{STEP}t{TICKERS}k{KNIVES}s{SLIP}m{MULTI}'
                        df.to_excel(writer, sheet_name=sheet_name)
                        # print(
                        #     f"Buy and hold  {hold_gain:.2f}    Dipper max draw_down {max_draw_down:.2f}   Rebal max draw_down {max_draw_down_rebal:.2f}")
                        df_row = pd.DataFrame(
                            [[STEP, start_date, TICKERS, KNIVES, SLIP, MULTI, hold_gain, hold_rebal_gain, dipper, max_draw_down_rebal, max_draw_down, max_loosing_weeks,
                              dipper/hold_gain, hold_gain/(end_index-start_index)*YEAR, hold_rebal_gain/(end_index-start_index)*YEAR, dipper/(end_index-start_index)*YEAR]], columns=df_tot.columns)
                        df_tot = df_tot.append(df_row)
                        print(df_row)
                        pass
writer.save()
df_tot.to_csv('results/dipper_tot.csv')
